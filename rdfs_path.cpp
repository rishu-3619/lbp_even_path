...
void rdfs_pathfinder(size_t vi, size_t ui, size_t dest, vector<bool> &visited, vector<Point> vertices, vector<vector<size_t>> &adj, vector<size_t> &path, vector<size_t> &ans){
    cout<< ui << " ";
    path.push_back(ui);
    visited[ui]=true;
    if(adj[ui].size()==0){
    ...
    for(int i=e1; i>=0; i--){
        // cout<<"i:"<<i<<endl;
        if (!visited[adj[ui][i]])
            rdfs_pathfinder(ui, adj[ui][i], dest, visited, vertices, adj, path, ans);
    }
...
vector<size_t> rdfs_call_pathfinder(vector<Point> vertices, size_t vi, size_t ui, size_t dest, vector<vector<size_t>> &adj){
    ...
    rdfs_pathfinder(vi, ui, dest, visited, vertices, adj, path, ans); 
    return ans; 
}
...
//     {}   // Vertex 5 is adjacent to vertex no one
// };
    vector<size_t> result= rdfs_call_pathfinder(vertices, 0, 1, 3, adj);
