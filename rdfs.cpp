...
void rdfs(size_t vi, size_t ui, vector<bool> &visited, vector<Point> vertices, vector<vector<size_t>> &adj, vector<size_t> &ans){
    cout<< ui << " ";
    ans.push_back(ui);
    visited[ui]=true;
    if(adj[ui].size()==0){
    ...
    for(int i=e1; i>=0; i--){
        // cout<<"i:"<<i<<endl;
        if (!visited[adj[ui][i]])
            rdfs(ui, adj[ui][i], visited, vertices, adj, ans);
    }
...
vector<size_t> rdfs_call(vector<Point> vertices, int vi, int ui, vector<vector<size_t>> &adj){
    ...
    ans.push_back(vi);
    visited[vi]=true;
    rdfs(vi, ui, visited, vertices, adj, ans);  
    return ans; 
}
